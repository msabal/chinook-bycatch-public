...
haul_dat <- readRDS(str_c(in_drive, "Raw Files/ashop_haul_info_20220211.rds"))
haul_dat_2021 <- readRDS(str_c(in_drive, "Raw Files/ashop_2021_haul_info_20220511.rds"))
...
haul_dat <- haul_dat %>% 
  mutate(date_haul_dep = as_date(deployment_date),
         date_haul_ret = as_date(retrieval_date),
         year = year(deployment_date),
         month = month(deployment_date),
         doy = yday(deployment_date),
         time_num = as.numeric(parse_hms(paste(hour(haul_dat$deployment_date), minute(haul_dat$deployment_date), "00", sep = ":"))),
         time_hms = parse_hms(paste(hour(haul_dat$deployment_date), minute(haul_dat$deployment_date), "00", sep = ":")),
...
haul_sf <- st_join(haul_sf, states2) %>% # join the states dataframe with spatial dataset of observer dataset.
...
haul_sf %>% 
  filter(over_land == "land") %>% 
  dplyr::select(haul_join, unique_haul_id, bottom_fa_to_m, fishing_fa_to_m, year_obs)
...
ggplot() + 
  ...
  geom_sf(data = haul_sf, shape = 21) +
  geom_sf(data = haul_sf %>% filter(over_land != 'ocean'), 
          size = 2, 
          color = 'red') + facet_wrap(~year_obs, ncol = 10) +
  coord_sf(xlim = range(haul_sf$londd_end), # note the coords_sf need to be at the end otherwise the last geom_sf overwrites the coordinate limits.
           ylim = range(haul_sf$latdd_end))
...
# Make new column in haul_dat with extracted bottom depth values for each haul.
haul_dat$bottom_depth_m_CRM <- -1* raster::extract(x = bath0,  # multiply by 1 to get depth as positive values.
                                                  y = haul_sf, # has to be a spatial vector object with geometry! haul_dat returns an error.
                                                  method = 'bilinear')

haul_sf$bottom_depth_m_CRM <- -1* raster::extract(x = bath0,  # multiply by 1 to get depth as positive values.
                                                 y = haul_sf, # has to be a spatial vector object with geometry! haul_dat returns an error.
                                                 method = 'bilinear')
...
haul_final <- filter(haul_final, unique_haul_id != "98126338352005")

# TOW DURATION
# N=31 hauls with NAs for tow duration -> FIX IT by calculating from deployment and retrieval dates and dividing by half.

# First, filter haul_final to get those rows with NAs in duration, then replace the duration column with the new values we calculated above, then add back into the haul_final (after filtering those same NA values)
new_dur <- haul_final %>%
...
haul_final <- haul_final %>% 
  ...
  rbind(filter(haul_final, !duration_in_min < 10))
...
haul_final <- haul_final %>% 
  ...
  rbind(filter(haul_final, !is.na(bottom_depth_fathoms)))
...
#### SAVE final haul_dat data.

saveRDS(haul_final, str_c(in_drive, "Saved Files/data_by_haul_v1.rds"))
